...
export default function Home() {
  return (
    <div className="flex flex-col items-center justify-center w-screen h-screen overflow-hidden bg-gradient-to-b from-black via-zinc-200/20 to-black">
        ...
        <ul className="flex items-center justify-center gap-4">
          {navigation.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="text-sm duration-500 font-semibold text-white hover:text-red-400 "
            >
              {item.name}
            </Link>
          ))}
